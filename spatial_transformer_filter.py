...
class SpatialTransformerFilter(Conv2D):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(SpatialTransformerFilter, self).__init__(
            rank=2,
            filters=filters,
...
#                 int(output_size[1]),
#                 int(input_shape[-1]))
        return super(Conv2D, self).compute_output_shape(self, input_shape)
    
    #this is where the layer's logic lives. When calling this, only the first argument matters which is the input
    def call(self, X, mask=None):
        # get the transformation matrix
        affine_transformation = self.locnet.call(X)
        output = self._transform(affine_transformation, X, self.output_size)
        convOutput = super(Con2D, self).call(self, X, mask=None)
        return convOutput
